...
public:
     ...
     * @param iniThFAST     初始 FAST 角点检测阈值（较严格，用于提取高质量角点）
     * @param minThFAST     最小 FAST 角点检测阈值（较宽松，当初始阈值提取不到角点时作为退化策略使用）
     */
     ...
     * @param image        输入图像矩阵
     * @param mask         掩码矩阵（指定只在特定区域提取特征）
     * @param keypoints    输出提取到的关键点集合
     * @param descriptors  输出计算得到的 ORB 描述子矩阵，维度为 $N \times 32$ (格式为 CV_8UC1)
     */
...
private:
    // 构建图像金字塔（生成多尺度图像以实现尺度不变性）
    void ComputePyramid(const cv::Mat& image); 
    
    // 遍历金字塔各层，基于四叉树策略提取 FAST 角点
    ...
    std::vector<float> mvLevelSigma2;          // 尺度方差（缩放因子的平方）
    std::vector<float> mvInvLevelSigma2;       // 尺度方差的倒数

    std::vector<cv::Point> pattern;            // 用于计算 BRIEF 描述子的随机采样点对模式（共 256 对，即 512 个点）
    std::vector<int> umax;                     // 用于计算灰度质心时，半径为 $R$ 的圆域内每一行（即不同 $v$ 坐标）对应的最大 $u$ 坐标偏移量
};
